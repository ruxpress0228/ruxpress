...
describe("bankTransfer API", () => {
  ...
  it("adminListLedgerEntries builds query string", async () => {
    ...
    await adminListLedgerEntries({ page: 1, size: 30, status: "PENDING", entryType: "ESCROW_HOLD", userId: 7 });

    expect(api.get).toHaveBeenCalledWith(
      "/v1/admin/bank-transfers?page=1&size=30&status=PENDING&entryType=ESCROW_HOLD&userId=7"
    );
  });
